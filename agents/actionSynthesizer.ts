...
export async function synthesizeAction(
  ...
  const { failureFacts, failureCategory, artifactSignals, selectorAnalysis } = input;
  ...
  if (heuristicResult && heuristicResult.confidence >= 0.8) {
    console.log('[ActionSynthesizer] Using high-confidence heuristic result:', heuristicResult);
...
function applyHeuristics(
  ...
  if (failureCategory.category === 'navigation_error') {
    return {
      ...
      urgency: 'high',
      reason: 'Navigation error indicates an application issue. Check server logs, network connectivity, and application health.',
    };
  }
  ...
  if (failureCategory.category === 'auth_error') {
    return {
      ...
      urgency: 'high',
      reason: 'Authentication error suggests an application or environment configuration issue. Verify credentials, session management, and auth service status.',
    };
  }
  ...
  if (
    ...
    (artifactSignals.uiState.includes('element missing') || artifactSignals.uiState.includes('element not found'))
  ) {
    // If selector is fragile, definitely test issue
    if (selectorAnalysis && (selectorAnalysis.selectorQuality === 'fragile' || selectorAnalysis.selectorQuality === 'poor')) {
      return {
        verdict: 'test_issue',
        recommendedAction: selectorAnalysis.suggestedSelector ? 'fix selector' : 'review selector strategy',
        urgency: 'medium',
    ...
    return {
      ...
      urgency: 'medium',
      reason: 'Page loaded successfully but element not found in DOM. This suggests the selector may be incorrect or the element structure changed.',
    };
  }
  ...
  if (
    ...
    !artifactSignals.blockingFactors.every(f => f.includes('No blocking factors'))
  ) {
    return {
      verdict: 'app_issue',
      recommendedAction: 'investigate app',
  ...
  if (
    ...
    artifactSignals &&
    (artifactSignals.pageState === 'loading' || artifactSignals.pageState === 'timeout')
  ) {
    return {
      verdict: 'app_issue',
      recommendedAction: 'increase timeout',
      urgency: 'medium',
      reason: 'Page failed to load within timeout period. This could indicate slow network, slow application response, or resource loading issues.',
    };
  }
  ...
  if (
    ...
    return {
      ...
      urgency: 'medium',
      reason: 'Assertion failed but page loaded correctly. This suggests the test expectations may be incorrect or the test logic needs review.',
    };
  }
  ...
  if (
    artifactSignals &&
    (artifactSignals.pageState === 'error' || artifactSignals.pageState === 'timeout') &&
    artifactSignals.blockingFactors.some(f => f.includes('network') || f.includes('error'))
  ) {
    return {
      verdict: 'app_issue',
      recommendedAction: 'investigate app',
