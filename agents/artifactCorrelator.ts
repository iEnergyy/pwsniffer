...
import {
	...
	type DOMSnapshot,
} from "@/tools/extractDOM";
import {
	detectPageLoadState,
	detectNavigationEvents,
	detectRedirects,
	type PageLoadState,
} from "@/tools/detectPageLifecycle";
import {
	analyzeScreenshot,
	type ScreenshotAnalysis,
} from "@/tools/analyzeScreenshot";
...
export async function correlateArtifacts(
	...
	try {
		...
		const blockingElements = domSnapshot
			? findBlockingElements(domSnapshot)
			: [];
		...
		return {
			...
			pageState: "unknown",
			blockingFactors: [
				`Error analyzing artifacts: ${error instanceof Error ? error.message : "Unknown error"}`,
			],
		};
	}
}
...
${
	screenshotAnalysis
		? `- Page State: ${screenshotAnalysis.pageState}
