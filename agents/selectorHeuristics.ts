...
import type { TestFailureFacts, FailureCategory, SelectorAnalysis } from '@/types/schemas';
...
export async function analyzeSelectorHeuristics(
  ...
  console.log('[SelectorHeuristics] Selector-related check:', {
    isSelectorRelated,
    category: failureCategory.category,
    failedStepContainsSelector: failureFacts.failedStep.toLowerCase().includes('selector'),
    failedStepContainsLocator: failureFacts.failedStep.toLowerCase().includes('locator'),
    errorContainsElement: failureFacts.error.toLowerCase().includes('element'),
    errorContainsSelector: failureFacts.error.toLowerCase().includes('selector'),
  });

  if (!isSelectorRelated) {
    console.log('[SelectorHeuristics] Not a selector-related failure, skipping');
  ...
  if (!extractedSelector) {
    console.log('[SelectorHeuristics] Trying to extract selector from error message...');
    extractedSelector = extractSelector(failureFacts.error);

    if (extractedSelector) {
      console.log('[SelectorHeuristics] Successfully extracted selector from error message:', {
        selector: extractedSelector.selector,
        type: extractedSelector.type,
        originalFormat: extractedSelector.originalFormat,
      });
    } else {
      console.log('[SelectorHeuristics] Failed to extract selector from error message');
    }
  } else {
    console.log('[SelectorHeuristics] Successfully extracted selector from failed step:', {
      selector: extractedSelector.selector,
      type: extractedSelector.type,
      originalFormat: extractedSelector.originalFormat,
    });
  ...
  if (!extractedSelector && input.failedAction?.action?.selector) {
    ...
    console.log('[SelectorHeuristics] Found selector in trace action:', traceSelector);

    // Determine selector type
    let selectorType: 'css' | 'playwright_locator' | 'text' | 'unknown';
    if (traceSelector.startsWith('#') || traceSelector.startsWith('.') || traceSelector.includes('[')) {
      selectorType = 'css';
    } else if (traceSelector.includes('getBy') || traceSelector.includes('locator')) {
    ...
    extractedSelector = {
      ...
      isPlaywrightAPI: traceSelector.includes('getBy') || traceSelector.includes('locator'),
    };
    console.log('[SelectorHeuristics] Created extracted selector from trace:', extractedSelector);
  ...
  if (!extractedSelector) {
    console.log('[SelectorHeuristics] No selector found - cannot analyze. Summary:', {
      failedStep: failureFacts.failedStep,
      error: failureFacts.error,
  ...
  const qualityAnalysis = analyzeSelectorQuality(
    selector,
    domSnapshot?.html
  );

  // Step 4: Suggest alternative selector if DOM available
  let selectorSuggestion: { suggestedSelector: string; reason: string; confidence: number } | null = null;
  if (domSnapshot) {
...
interface SynthesisInput {
  ...
  selectorSuggestion: { suggestedSelector: string; reason: string; confidence: number } | null;
  failureFacts: TestFailureFacts;
  failureCategory: FailureCategory;
}
