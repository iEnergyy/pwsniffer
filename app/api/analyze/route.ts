    ...
    if (zipFile && isZipFile(zipFile)) {
      ...
      if (!extracted.reportJson) {
        return NextResponse.json(
          { error: 'Could not find a Playwright report JSON file in the ZIP file. Please ensure a JSON file with Playwright test results is included.' },
          { status: 400 }
        );
      }
      ...
      if (!extracted.traceZip) {
        return NextResponse.json(
          { error: 'Could not find a trace ZIP file in the ZIP file. Please ensure a trace.zip or similar trace archive is included.' },
          { status: 400 }
        );
      }
      ...
      if (!reportFile) {
        return NextResponse.json(
          { error: 'Missing required file: Playwright report JSON file or ZIP file containing artifacts' },
          { status: 400 }
        );
      }
      ...
      if (!traceFile) {
        return NextResponse.json(
          { error: 'Missing required file: Trace ZIP file or ZIP file containing artifacts' },
          { status: 400 }
        );
      }
