...
async function checkTraceAvailability(sessionId: string): Promise<{ available: boolean; expired?: boolean }> {
  ...
  try {
    ...
    if (!available) {
      return NextResponse.json(
        { error: expired ? 'Session expired' : 'Trace not found or session expired' },
