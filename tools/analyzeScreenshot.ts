...
const ScreenshotAnalysisSchema = z.object({
	pageState: z
		.enum(["loaded", "loading", "error", "blank", "unknown"])
		.describe("State of the page in the screenshot"),
	blockingElements: z
		.array(z.string())
		.describe(
			"List of blocking UI elements detected (modals, banners, overlays, etc.)",
		),
	visibleContent: z
		.array(z.string())
		.describe("List of visible content elements or text on the page"),
	confidence: z
		.number()
		.min(0)
		.max(1)
		.describe("Confidence score for this analysis"),
});
...
export async function analyzeScreenshot(
	...
	try {
		...
		const mimeType = screenshot instanceof File ? screenshot.type : "image/png";
