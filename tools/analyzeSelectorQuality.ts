...
export function analyzeSelectorQuality(
  ...
  if (type === 'playwright_locator' && isPlaywrightAPI) {
    ...
    if (selector.includes(' > ')) {
      ...
      if (depth >= 3) {
        issues.push(`Deep nesting detected (${depth} levels) - fragile to DOM structure changes`);
        score -= 0.2;
      } else if (depth >= 2) {
        issues.push(`Moderate nesting (${depth} levels) - may break with layout changes`);
        score -= 0.1;
      }
    }
  ...
  if (selector.includes('random') || selector.includes('uuid') || selector.includes('id-')) {
  ...
  if (type === 'css') {
    ...
    if (/^[a-z]+$/.test(selector.trim())) {
      issues.push('Selector is too generic (single tag name) - may match multiple elements');
      score -= 0.3;
    }
  ...
  if (domHtml && type === 'css') {
    ...
    if (tagMatch) {
      ...
      const tagCount = (domHtml.match(new RegExp(`<${tagName}[^>]*>`, 'gi')) || []).length;
      if (tagCount > 10) {
        issues.push(`Many ${tagName} elements found in DOM - selector may not be unique`);
        score -= 0.1;
      }
    }
  }
  ...
  const antiPatterns = [
    { pattern: /body > /i, issue: 'Selector starts from body - overly specific' },
    { pattern: /html > /i, issue: 'Selector starts from html - overly specific' },
    { pattern: /:nth-child\(\d+\)/i, issue: 'Uses nth-child - fragile to DOM order changes' },
    { pattern: /:first-child|:last-child/i, issue: 'Uses positional pseudo-selectors - fragile' },
  ];
