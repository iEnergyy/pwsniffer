...
export interface PatternMatchResult {
  category: 'selector_not_found' | 'timeout' | 'assertion_failed' | 'navigation_error' | 'auth_error' | 'unknown';
  confidence: number;
 ...
 */
export function applyPatternMatching(facts: TestFailureFacts): PatternMatchResult | null {
  ...
  const matches: Array<{ category: PatternMatchResult['category']; score: number; patterns: string[] }> = [];
  ...
  if (selectorMatches.length > 0) {
    matches.push({
      ...
      score: selectorMatches.length * 0.15 + (errorLower.includes('element') ? 0.2 : 0),
  ...
  const assertionMatches = assertionPatterns.filter(p => p.test(combinedText));
  ...
  const navigationMatches = navigationPatterns.filter(p => p.test(combinedText));
