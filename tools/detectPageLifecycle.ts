...
export function detectPageLoadState(traceData: TraceData): PageLoadState {
  ...
  const documentRequests = networkEvents.filter(e => 
    e.type === 'request' && 
    (e.url.endsWith('.html') || 
  ...
  const loadEvents = actions.filter(a => 
    a.action?.name === 'load' || 
    a.action?.name === 'domcontentloaded' ||
    a.type === 'event' && (a.metadata?.name === 'load' || a.metadata?.name === 'DOMContentLoaded')
  );

  // Check for network errors
  const failedRequests = networkEvents.filter(e => 
    e.type === 'requestFailed' || 
    (e.type === 'response' && e.status && (e.status >= 400 || e.status === 0))
  );
  ...
  for (const request of failedRequests) {
    loadState.failedRequests.push({
      ...
      error: request.error || `HTTP ${request.status} ${request.statusText || ''}`,
    });
  ...
  if (loadState.networkErrors.length > 0) {
    ...
    const domContentLoaded = loadEvents.find(e => 
    ...
    const recentRequests = networkEvents.filter(e => 
      e.timestamp > (Date.now() - 10000) // Last 10 seconds
    );
  ...
  const timeoutActions = actions.filter(a => 
 ...
 */
export function detectNavigationEvents(traceData: TraceData): NavigationEvent[] {
