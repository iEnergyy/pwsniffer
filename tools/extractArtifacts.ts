 ...
 * - context.md (optional)
 */
export async function extractArtifactsFromZip(zipFile: File | Buffer): Promise<ExtractedArtifacts> {
  const zipBuffer = zipFile instanceof File 
  ...
  for (const entry of entries) {
    ...
    if (entryName.endsWith('.json')) {
      ...
      try {
        ...
        if (parsed.suites || parsed.config) {
          ...
          if (!artifacts.reportJson || (parsed.suites && parsed.suites.length > 0)) {
            artifacts.reportJson = Buffer.from(entryData);
            artifacts.reportFileName = originalEntryName.split('/').pop() || originalEntryName;
          }
        }
      } catch {
        // Not valid JSON, skip
    ...
    if (entryName.endsWith('.zip')) {
      ...
      if (entryName.includes('trace') || !artifacts.traceZip) {
        ...
        artifacts.traceFileName = originalEntryName.split('/').pop() || originalEntryName;
      }
    }
    ...
    // Identify context.md
    if (entryName.includes('context.md') || 
