...
export interface BlockingElement {
  type: 'modal' | 'overlay' | 'banner' | 'spinner' | 'error' | 'auth' | 'cookie' | 'unknown';
  selector: string;
  description: string;
  confidence: number;
}
 ...
 */
function findElementInHTML(html: string, selector: string): { found: boolean; html: string; style: string; classes: string } | null {
  // Try ID selector (#id)
...
export function checkElementVisibility(
  ...
  try {
    ...
    return {
      exists: true,
      visible: !isHidden,
      reason: isHidden ? 'Element exists but is hidden' : 'Element exists and appears visible',
      computedStyles: {
      ...
      for (const regex of pattern.regex) {
        ...
        if (matches) {
          ...
          if (matchIndex !== -1) {
            ...
            const beforeMatch = dom.html.substring(Math.max(0, matchIndex - 200), matchIndex);
            ...
            if (tagStart !== -1 && tagEnd !== -1) {
              ...
              if (!isHidden) {
                ...
                if (isBlocking || pattern.type === 'spinner' || pattern.type === 'error') {
    ...
    for (const element of blockingElements) {
      const existing = unique.find(
        e => e.type === element.type &&
          e.description.substring(0, 50) === element.description.substring(0, 50)
      );
      if (!existing) {
        unique.push(element);
      }
