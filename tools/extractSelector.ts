...
export function extractSelector(failedStep: string): ExtractedSelector | null {
  console.log('[extractSelector] Attempting to extract selector from:', failedStep);
  ...
  if (textToSearch.includes('Locator:')) {
    ...
    if (locatorMatch) {
      ...
      console.log('[extractSelector] Found "Locator:" prefix, extracted:', textToSearch);
    }
  }
  ...
  const playwrightLocatorPatterns = [
    { name: 'getByRole', pattern: /getByRole\s*\(\s*['"]([^'"]+)['"]\s*(?:,\s*\{[^}]*\})?\s*\)/i },
    { name: 'getByText', pattern: /getByText\s*\(\s*['"]([^'"]+)['"]\s*\)/i },
    { name: 'getByLabel', pattern: /getByLabel\s*\(\s*['"]([^'"]+)['"]\s*\)/i },
    { name: 'getByPlaceholder', pattern: /getByPlaceholder\s*\(\s*['"]([^'"]+)['"]\s*\)/i },
    { name: 'getByAltText', pattern: /getByAltText\s*\(\s*['"]([^'"]+)['"]\s*\)/i },
    { name: 'getByTitle', pattern: /getByTitle\s*\(\s*['"]([^'"]+)['"]\s*\)/i },
    { name: 'getByTestId', pattern: /getByTestId\s*\(\s*['"]([^'"]+)['"]\s*\)/i },
  ...
  for (const { name, pattern } of playwrightLocatorPatterns) {
    ...
    if (match) {
      ...
      console.log(`[extractSelector] ✓ Matched ${name} pattern:`, { selector, fullMatch });
      
      // Determine the Playwright API type
      const apiType = fullMatch.match(/getBy(\w+)|locator/i)?.[1]?.toLowerCase() || 'locator';
  ...
  const cssSelectorPatterns = [
    ...
    { name: 'Attribute selector ([attr=value])', pattern: /\[([\w-]+)(?:=["']([^"']+)["'])?\]/ },
    { name: 'Complex CSS (div > span)', pattern: /([\w-]+\s*(?:[>+~]\s*[\w-]+|\.[\w-]+|#[\w-]+)+)/ },
  ];
  ...
  if (textMatch) {
    ...
    if (selector.length < 50 && !selector.includes('Error') && !selector.includes('failed')) {
      ...
      console.log('[extractSelector] ✗ Quoted string rejected (too long or contains error keywords)');
    }
  } else {
  ...
  console.log('[extractSelector] ✗ No selector found after trying all patterns');
  return null;
}
...
export function extractSelectors(failedStep: string): ExtractedSelector[] {
  ...
  for (const match of quotedStrings) {
    ...
    if (additional && !selectors.some(s => s.selector === additional.selector)) {
