 ...
 */
export async function readTraceZip(traceZip: File | Buffer): Promise<TraceData> {
  const zipBuffer = traceZip instanceof File 
  ...
  const nestedZipEntry = entries.find(entry => 
    !entry.isDirectory && 
  ...
  if (nestedZipEntry && nestedZipEntry.entryName.toLowerCase().endsWith('.zip')) {
    try {
  ...
  let traceEntry = entries.find(entry => 
  ...
  if (!traceEntry) {
    traceEntry = entries.find(entry => 
      entry.entryName.endsWith('.trace') && !entry.isDirectory
    );
  }
  ...
  if (!traceEntry) {
    traceEntry = entries.find(entry => 
      !entry.isDirectory && 
      (entry.entryName.includes('trace') || entry.entryName.includes('Trace'))
    );
  }
  ...
  if (!traceEntry) {
    ...
    throw new Error(`trace.trace file not found in trace.zip. Found ${entries.length} entries (${entries.filter(e => !e.isDirectory).length} files). First few: ${entryNames.join(', ')}`);
  }
  ...
  for (const line of traceLines) {
    try {
      ...
      if (event.type === 'resource' || event.type === 'request' || event.type === 'response') {
        const networkEvent: NetworkEvent = {
          ...
          headers: event.headers || event.request?.headers || event.response?.headers,
          error: event.error,
        };
  ...
  for (const entry of entries) {
    if (entry.entryName.startsWith('snapshots/') && entry.entryName.endsWith('.html')) {
      const snapshotId = entry.entryName
        .replace('snapshots/', '')
        .replace('.html', '');
      
      const snapshot = traceData.snapshots.find(s => s.snapshotId === snapshotId);
      if (snapshot) {
