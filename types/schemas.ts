...
const TestFailureFactsSchemaForAI = z.object({
	...
	timeout: z
		...
		.nullable()
		.describe(
			"Timeout value in milliseconds if applicable, or null if not available",
		),
	lineNumber: z
		.number()
		.nullable()
		.describe(
			"Line number where the failure occurred, or null if not available",
		),
	columnNumber: z
		.number()
		.nullable()
		.describe(
			"Column number where the failure occurred, or null if not available",
		),
	stackTrace: z
		.array(z.string())
		.nullable()
		.describe("Stack trace lines, or null if not available"),
});
...
export const TestFailureFactsSchema = z.object({
	...
	timeout: z
		.number()
		.optional()
		.describe("Timeout value in milliseconds if applicable"),
	lineNumber: z
		.number()
		.optional()
		.describe("Line number where the failure occurred"),
	columnNumber: z
		.number()
		.optional()
		.describe("Column number where the failure occurred"),
...
export const PlaywrightArtifactsSchema = z.object({
	reportJson: z
		...
		.describe("Playwright JSON report (required)"),
	traceZip: z
		.union([z.instanceof(File), z.instanceof(Buffer)])
		.optional()
		.describe("Playwright trace.zip (required)"),
	screenshots: z
	...
	contextMd: z
		.string()
		.optional()
		.describe("Optional manual context provided by user"),
});
...
export const ArtifactSignalsSchema = z.object({
	...
	pageState: z
		.string()
		.describe("State of the page (loaded, loading, error, etc.)"),
	blockingFactors: z
		.array(z.string())
		.describe("Factors that may have blocked the test"),
});
...
export const FinalDiagnosisSchema = z.object({
	verdict: z
		.enum(["test_issue", "app_issue", "unclear"])
		.describe("Whether this is a test issue or application issue"),
...
export const SelectorAnalysisSchema = z.object({
	selectorQuality: z
		.enum(["excellent", "good", "fragile", "poor"])
		.describe("Quality rating of the selector"),
	qualityScore: z
		.number()
		.min(0)
		.max(1)
		.describe("Numeric quality score (0-1)"),
	issues: z
		.array(z.string())
		.describe("List of issues found with the selector"),
	suggestedSelector: z
		.string()
		.nullable()
		.describe("Suggested alternative selector, or null if current is good"),
	suggestionReason: z
		.string()
		.nullable()
		.describe("Explanation of why the suggestion is better"),
...
export const SolutionSuggestionSchema = z.object({
	suggestedCode: z
		...
		.nullable()
		.describe(
			"Suggested code fix (selector, test logic, etc.) or null if not applicable",
		),
	originalCode: z
		.string()
		.nullable()
		.describe("Original code that failed, or null if not extractable"),
	explanation: z.string().describe("Explanation of the suggested fix"),
	steps: z
		.array(z.string())
		.describe("Step-by-step instructions for implementing the fix"),
	alternativeApproaches: z
		.array(z.string())
		.optional()
		.describe("Alternative approaches if the primary solution doesn't work"),
	confidence: z
		.number()
		.min(0)
		.max(1)
		.describe("Confidence in the solution (0-1)"),
});
